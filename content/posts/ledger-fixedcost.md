...
ところがこれを仮想アカウント＝ `FixedCost` でクエリすると、3月に支払った3,000円が1月分と2月分に1,500円ずつに按分されません(注[^fn:2])。これではaux-dateを使用しているユーザは困ります(注[^fn:3]）。
...
ならばと、従来から使っていた「特別費・経常費の推移チャート」に固定費を重ね書きしたのが図[1](#org0862c7b)です(注[^fn:5])。赤い折れ線が固定費の推移、青い棒グラフが経常費、ピンクの棒グラフは特別費です。
...
[^fn:3]: 仮想アカウントがaux-dateをrespectしてくれるようにする方法法があるような気がします。しかし現在のところ検索しても解決策が見当たりません。どなたか教えていただけるとありがたいです。
