...
: 日本郵便のデータをダウロードし `postal-coder.sh` 用にデータを整形します（おおざっぱなフローは[こちら](#org66d19d5)）。
...
データに存在しない郵便番号の場合には `** Not found.` を表示して停止します。
...
アップデートがなければほとんど何もせずスクリプトは終わります（図[1](#org66d19d5)）。
