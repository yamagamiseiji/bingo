...
tags = ["commodity", "Ledger-cli", "gasoline"]
categories = ["comp"]
draft = false
+++
...
そこで次のように、ガソリンの給油量単位（L；リッター）をLedgerの ****コモディティ****&nbsp;[^fn:1] にし、その後ろに `@` をつけて単価を書きます[^fn:2]：
...
[^fn:1]: Ledgerのコモディティの一般的な使い方はドルやユーロなどの ****通貨単位**** ですが、この例のように `L` にしたりできる柔軟性があります。
[^fn:2]: この修正を手作業で行うと必ずミスが起こります。簡単なbashスクリプトを書いて書き換えましょう。
