...
#           Time-stamp: <2022-01-07 19:07:49 yamagami>
...
* つくったわけ
年末に年賀状の宛名データが壊れました[fn:バックアップは]。
それを復旧させる作業をやっていて、
郵便番号と住所のどちらからでもお互いに検索できる道具が欲しくなりました[fn:市販ソフト]。

そこで大急ぎで作ったのが *=postal-coder.sh=* 。
とても単純で原始的ですが、
とりあえずこれで過去3年間の年賀状をめくりながら、
宛名ファイルを作り直し、宛名印刷を完了することができました[fn:csvベース]。

宛名印刷が終わった後、
日本郵便の郵便番号データベースは月に1回のペースで更新されることを知りました。
そこで毎月定期的に郵便番号データをダウンロードし
...
[fn:市販ソフト] 市販の住所録・宛名印刷ソフトでは備わっているのが当たり前のありきたりな機能です。

[fn:csvベース] 残っていたCSVファイルをEmacsのCSVモードで読み込み、それに追記・修正を加えるという形で行いました。復旧したデータも当然CSVです。

* スクリプト  
- =postal-coder.sh= :: 住所から郵便番号、郵便番号から住所、事業所名から郵便番号と住所をゲットする。コードは [[#postal-coder-code][こちら]]。

- =fabr-postalcode-DB.sh= :: 日本郵便のデータベースをダウロードし =postal-coder.sh= 用にデータを整形する。コードは [[#fabr-postalcode-code][こちら]]。

* スクリプト postal-coder.sh
** 使用例
*** 住所 *→* 郵便番号  
#+begin_example
...
ませんでした。ダウンロードを省略します。
  #+end_src
もし更新があった場合、一般郵便番号データは 1.7M 事業所名データは 0.7M 程度。
回線速度次第だが、まぁそんなに大きな負担ではない。
...
** fabr-postalcode-DB
    :PROPERTIES:
    :custom_id: fabr-postalcode-code
    :END:
    #+begin_src
#!/bin/bash
