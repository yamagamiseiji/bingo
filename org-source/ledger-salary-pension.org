...
#           Time-stamp: <2021-10-27 17:03:36 yamagami>
...
- Ledgerでは１トランザクション内に複数の通貨（JPY, $, &euro; など)を混在させることができます。当日の通貨の交換レートを為替レートデータベース（ディフォルトでは =.prices.db= ファイル）から読み込んで換算してくれます。そのためトランザクション内では *通貨単位* （上の例では =JPY= ）は *必須* です。
